...
flowchart TB
  subgraph Libraries["Library packages (src/)"]
    ...
    Comp["TrBlazeUI.Components<br/>(79 styled component families)"]
...
**`TrBlazeUI.Components` — the styled layer.** **79 component families** (69 through 2.0.1 — 68 catalogued in the structure doc plus the later-added **Toolbar** family — plus the nine added in 2.1.0: `Prose`, `Stat`, `Timeline`, `Stepper`, `AnchorNav`, `SortableList`, `PasswordStrength`, `CodeBlock`, `CenteredPanel`; 344 public component types in total) compose the primitives and add Tailwind-based styling driven entirely by CSS variables, so any shadcn/ui or tweakcn theme works unmodified. Components follow a feature-folder layout (each component in its own folder with `.razor` markup, `.razor.cs` logic, and enum/config classes) and a code-behind pattern. Multi-part components compose many sub-components (Dialog = 8 parts, Sidebar = 22, Sheet/DropdownMenu/AlertDialog/Command ≈ 8–9 each). The project bundles the pre-built `trblazeui.css` (~906 KB minified / ~97 KB gzipped as of 2.1.0, compiled by the standalone Tailwind binary from `trblazeui-input.css`) so consumers need no Tailwind toolchain. As of 2.1.0 that bundle carries the **standard Tailwind utility scale with responsive variants**, not only the utilities the library's own components reference — a tree-shaken bundle made the library's own "style application markup with utility classes" guidance silently false for consumers (TechieBlog TR-019). Rich-content components pull in third-party engines: Chart → Blazor-ApexCharts, MarkdownEditor → Markdig, RichTextEditor → Quill.js (via JS interop) with HtmlSanitizer guarding the HTML output. A key cross-cutting invariant — surfaced by three consumer issue reports and completed in 2.1.0 — is that **every** public component implements `[Parameter(CaptureUnmatchedValues = true)]` attribute splatting so arbitrary HTML attributes and event handlers (`id`, `style`, `@onkeydown`, …) pass through to the rendered element rather than throwing at runtime. This is now measured rather than asserted: `tools/splat-audit` reflects over the built assemblies and reports 344/344 in Components and 59/59 in Primitives. Components that own no element of their own (the cascading-context roots and the config-only `DataTableColumn`) accept the attributes without rendering them; that exception list is published in the AI reference.
