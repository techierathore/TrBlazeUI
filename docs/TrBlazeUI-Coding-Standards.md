...
> **Instance-field convention for this project: `obj` prefix.** Static analysis of the codebase shows the `obj`-prefix style dominates (`objLogger`, `objJsRuntime`, `objModule`, …) and the .NET 10 modernization pass reports 0 underscore-prefixed fields remaining. This is the pinned, authoritative choice.

## Database Naming Conventions

> TrBlazeUI is a UI component library with no database. These conventions apply to any consumer/back-end code written under these standards.
...
|------|-----------|---------|
| **Instance fields** | `obj` prefix + PascalCase tail (no underscores) | `private readonly ILogger<X> objLogger;`<br>`private readonly IJSRuntime objJsRuntime;`<br>`private IJSObjectReference? objModule;` |
...
| **Booleans** | same prefix + `Is`/`Has`/`Can` | `IsAuthenticated`, `vIsValid`, `aHasAccess`, `objIsBold` |
...
**Blazor-specific:** `[Parameter]`, `[CascadingParameter]`, and `[Inject]` properties are PascalCase with NO `obj` prefix (they are properties, not fields). Private `EventCallback`/`RenderFragment` *fields* follow the `obj` rule; in `.razor` markup, a private field `objField` is referenced as `@objField`.
...
**PascalCase, no separators.** `TrBlazeUIBaseUrl` NOT `TRBLAZEUI_BASE_URL` and NOT `TrBlazeUI__BaseUrl`. Use a custom configuration provider mapping PascalCase env vars → `:`-nested config paths. Read via `IConfiguration["Section:Key"]` only — never `Environment.GetEnvironmentVariable(...)`.

### File Structure
```csharp
using System;

namespace TrBlazeUI.Components.Example;

public class ExampleService
{
    private readonly ILogger<ExampleService> objLogger;
    private readonly IConfiguration objConfiguration;

    public ExampleService(ILogger<ExampleService> aLogger, IConfiguration aConfiguration)
    {
        objLogger = aLogger;
    ...
    public async Task<DataTable> GetDataAsync(string aQueryName)
    {
        ...
        var vResult = await ExecuteQueryAsync(vConnString, aQueryName).ConfigureAwait(false);
        return vResult;
    }
}
```
...
- `ConfigureAwait(false)` in libraries (`src/`); NOT in Blazor `.razor.cs` component code.
...
`<summary>`, `<remarks>`, `<param>`, `<returns>`, `<exception>` — all required. (`<remarks>` should give step-by-step logic for methods.) The project ships with `GenerateDocumentationFile=true` and currently suppresses `CS1591`; the suppression is removed once coverage reaches 100%.
...
### Security
- Never hardcode credentials. Parameterized queries. Validate inputs. Log security events. Sanitize user-supplied HTML (the library uses HtmlSanitizer in RichTextEditor).

### Blazor component testability — stable `data-testid` (Blazor screens)
- Interactive/data-bound controls the verifier must reach carry a stable `data-testid` (or stable element `id`) so Playwright selectors don't drift. Name by intent (`data-testid="theme-toggle"`), never positional. (Components already use `data-slot="..."` attributes for CSS targeting; add `data-testid` where a gate must address a specific element.)
...
# Field missing obj prefix (obj-style project)
